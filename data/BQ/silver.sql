-- AQUÍ IMPLEMENTAREMOS TANTO LA LÓGICA SCD2 COMO EL MODELO DE DATOS COMÚN (CDM) PARA LAS TABLAS SILVER

-- 1. Crear la tabla departments combinando los datos del Hospital A y B
...
-- 2. Truncar la tabla Silver antes de insertar
TRUNCATE TABLE `avd-databricks-demo.silver_dataset.departments`;

-- 3. Carga completa insertando los datos combinados
...
-- 1. Crear la tabla providers combinando los datos del Hospital A y B
...
-- 1. Crear la tabla patients en BigQuery
...
--Crear una tabla temporal quality_checks
...
-- 3. Aplicar la lógica SCD Tipo 2 con MERGE
...
-- Paso 1: Marcar los registros existentes como históricos si alguna columna cambió
WHEN MATCHED AND (
...
-- Paso 2: Insertar los registros nuevos y actualizados como los últimos registros activos
WHEN NOT MATCHED
THEN INSERT (
    Patient_Key,
    SRC_PatientID,
...
-- ELIMINAR la tabla quality_check
DROP TABLE IF EXISTS `avd-databricks-demo.silver_dataset.quality_checks`;

-------------------------------------------------------------------------------------------------------

-- 1. Crear la tabla transactions en BigQuery
...
-- 2. Crear una tabla temporal quality_checks
CREATE OR REPLACE TABLE `avd-databricks-demo.silver_dataset.quality_checks` AS
SELECT DISTINCT
...
-- 4. ELIMINAR la tabla quality_check
DROP TABLE IF EXISTS `avd-databricks-demo.silver_dataset.quality_checks`;

-------------------------------------------------------------------------------------------------------

-- 1. Crear la tabla encounters en BigQuery
...
-- 2. Crear una tabla temporal quality_checks para encounters
...
-- 1. Crear la tabla Claims en BigQuery
...
-- 2. Crear una tabla temporal quality_checks para claims
...
-- 1. Crear la tabla Silver de CP Codes en BigQuery
...
-- 2. Crear una tabla temporal quality_checks para CP Codes
...
SELECT
    ...
    code_status,
    datasource,
    -- Definir la condición de cuarentena (valores nulos en campos clave)
    CASE
