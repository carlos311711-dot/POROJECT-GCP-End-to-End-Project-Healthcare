...
> [!NOTE]
> ### 📍 Ubicación del Código de la Pipeline de CI/CD
> La definición del pipeline de integración y despliegue continuo de Google Cloud Build se encuentra en:
> * **Pipeline YAML:** [cloudbuild.yaml](../cloudbuild.yaml)
>
> ### ⚙️ Cómo Ejecutar
> Este pipeline se ejecuta automáticamente en GCP mediante un Trigger de Cloud Build ante cualquier `git push` a la rama `main` de GitHub. También puedes disparar la compilación manualmente desde tu consola local usando la herramienta `gcloud`:
> ```bash
...
El pipeline de CI/CD automatiza los pasos desde la máquina del desarrollador local hasta la ejecución final en la nube de GCP:

![Flujo de Trabajo CI/CD en GCP](../Picture/flow_github_cloudbuild_cloudcomposer.png)
...
Una vez que el código se escribe en tu entorno local y se empuja (**Git Push**) a una rama de desarrollo en tu repositorio de GitHub, un Pull Request para fusionar en la rama principal (`main`) o un push directo activa un Job de **Cloud Build** para ejecutar la sincronización automática de tus DAGs y scripts con tu entorno de **Cloud Composer (Airflow)**.

### Preparación del Entorno Operativo:
Para implementar con éxito este pipeline, debemos seguir esta secuencia de preparación:
1. **a. Crear el Entorno de Cloud Composer:** (Ver detalle en la [Guía 6 de Orquestación](file:///c:/Users/JhonCarlosJeanPaulVe/OneDrive%20-%20Grupo%20Alicorp/Escritorio/Home/3.Learn/POROJECT-GCP-End-to-End-Project-Healthcare/MANUAL/6_ORQUESTACION_WORKFLOW_AIRFLOW.md)).
2. **b. Configurar el Repositorio GitHub:**
   * Repositorio de referencia para entrenamiento/desarrollo: `https://github.com/saidhuljohny2/gcp-retailer-project.git` (o el repositorio personalizado de tu organización).
   * Clona el repositorio localmente en tu terminal para iniciar el desarrollo del pipeline.
3. **c. Crear el Disparador (Trigger) en Cloud Build:** (Ver pasos de configuración de parámetros del Trigger en la Sección 5).
...
El disparador es el "oyente" que ejecutará nuestro pipeline al detectar cambios de código en tu repositorio de GitHub:

1. Dentro de Cloud Build > Disparadores, haz clic en **Crear Disparador**.
2. Configura los parámetros del trigger de sincronización de DAGs:
   - **Nombre:** `add-dags-to-composer`
   - **Región:** `us-central1`
   - **Descripción:** `DAG sync job`
   - **Evento:** *Push a una rama (Push to a branch)*
3. Configura el origen del código:
   - **Repositorio (Source - Repository):** Selecciona tu repositorio de GitHub previamente conectado.
   - **Rama Base (Source - Base branch):** `^main$` (cambia `main` por el nombre de la rama base de tu repositorio, si fuera requerido).
4. Configura la construcción:
   - **Configuración:** Selecciona *Archivo de configuración de Cloud Build (yaml o json)*.
   - **Ubicación del archivo:** `cloudbuild.yaml` (alojado en la raíz de tu proyecto).
...
## 🔍 Lista de Validación de Integración Continua (Prueba de Funcionamiento)

¡Eso es todo! El flujo de trabajo ha sido completamente automatizado. Para confirmar que el pipeline de CI/CD está operando de manera óptima, sigue estos pasos de prueba:

* **1. Modificar un DAG localmente:** Realiza un cambio sencillo en un archivo Python de DAGs (ej. añadir un comentario o una tarea) y haz push de ese cambio a una rama de desarrollo en tu repositorio de GitHub.
* **2. Verificar el Job de Cloud Build:** Abre la pestaña **Historial** de Cloud Build en la consola de GCP y verifica que se haya disparado y completado exitosamente la ejecución asociada al commit.
* **3. Comprobar la Actualización de DAGs:** Ve a la carpeta `/dags/` en la consola de Cloud Storage o abre la interfaz de Airflow para verificar que las modificaciones se hayan sincronizado de inmediato.
